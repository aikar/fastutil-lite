...
public final class IntBigArrays {
	...
	private static final class BigArrayHashStrategy implements Hash.Strategy<int[][]>, java.io.Serializable {
	 ...
	 @Override
	 public int hashCode(final int[][] o) { return java.util.Arrays.deepHashCode(o); }
	 @Override
	 public boolean equals(final int[][] a, final int[][] b) { return IntBigArrays.equals(a, b); }
	}
	/** A type-specific content-based hash strategy for big arrays.
	 *
	...
	private static void selectionSort(final int[][] a, final long from, final long to, final IntComparator comp) {
	 for(long i = from; i < to - 1; i++) {
	  long m = i;
	  for(long j = i + 1; j < to; j++) if (comp.compare(IntBigArrays.get(a, j), IntBigArrays.get(a, m)) < 0) m = j;
	  if (m != i) swap(a, i, m);
	 }
	}
	...
	private static void selectionSort(final int[][] a, final long from, final long to) {
	 for(long i = from; i < to - 1; i++) {
	  long m = i;
	  for(long j = i + 1; j < to; j++) if (( (IntBigArrays.get(a, j)) < (IntBigArrays.get(a, m)) )) m = j;
	  if (m != i) swap(a, i, m);
	 }
	}
	...
	public static void radixSort(final int[][] a, final long from, final long to) {
	 ...
	 while(offsetPos > 0) {
	  ...
	  // Count keys.
	  for(long i = length; i-- != 0;) ByteBigArrays.set(digit, i, (byte)((((IntBigArrays.get(a, first + i)) >>> shift) & DIGIT_MASK) ^ signMask));
	  ...
	  for(long i = 0, d; i < end; i += count[c], count[c] = 0) {
	   int t = IntBigArrays.get(a, i +first);
	...
	private static void selectionSort(final int[][] a, final int[][] b, final long from, final long to) {
	 for(long i = from; i < to - 1; i++) {
	  ...
	  for(long j = i + 1; j < to; j++)
	   if (( (IntBigArrays.get(a, j)) < (IntBigArrays.get(a, m)) ) || ( (IntBigArrays.get(a, j)) == (IntBigArrays.get(a, m)) ) && ( (IntBigArrays.get(b, j)) < (IntBigArrays.get(b, m)) )) m = j;
	  if (m != i) {
	...
	public static void radixSort(final int[][] a, final int[][] b, final long from, final long to) {
	 final int layers = 2;
	 if (IntBigArrays.length(a) != IntBigArrays.length(b)) throw new IllegalArgumentException("Array size mismatch.");
	 ...
	 while(offsetPos > 0) {
	  ...
	  final int shift = (DIGITS_PER_ELEMENT - 1 - level % DIGITS_PER_ELEMENT) * DIGIT_BITS; // This is the shift that extract the right byte from a key
	  // Count keys.
	  for(long i = length; i-- != 0;) ByteBigArrays.set(digit, i, (byte)((((IntBigArrays.get(k, first + i)) >>> shift) & DIGIT_MASK) ^ signMask));
	  for(long i = length; i-- != 0;) count[ByteBigArrays.get(digit, i) & 0xFF]++;
	  // Compute cumulative distribution and push non-singleton keys on stack.
	  int lastUsed = -1;
	  long p = 0;
	  ...
	  for(long i = 0, d; i < end; i += count[c], count[c] = 0) {
	   int t = IntBigArrays.get(a, i + first);
	   int u = IntBigArrays.get(b, i + first);
	   c = ByteBigArrays.get(digit, i) & 0xFF;
	   while((d = --pos[c]) > i) {
	    int z = t;
	    final int zz = c;
