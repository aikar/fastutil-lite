...
/** A type-specific hash map with a fast, small-footprint implementation.
	...
	* @see HashCommon
	*/
public class Float2ObjectOpenHashMap <V> extends AbstractFloat2ObjectMap <V> implements java.io.Serializable, Cloneable, Hash {
	...
	final class MapEntry implements Float2ObjectMap.Entry <V>, Map.Entry<Float, V> {
	 ...
	 public boolean equals(final Object o) {
	  ...
	  return ( Float.floatToIntBits(key[index]) == Float.floatToIntBits((e.getKey()).floatValue()) ) && java.util.Objects.equals(value[index], (e.getValue()));
	 }
	 @Override
	 public int hashCode() {
	  return it.unimi.dsi.fastutil.HashCommon.float2int(key[index]) ^ ( (value[index]) == null ? 0 : (value[index]).hashCode() );
	 }
	 @Override
	 ...
	 public int nextEntry() {
	  ...
	  for(;;) {
	   if (--pos < 0) {
	    ...
	    while (! ( Float.floatToIntBits(k) == Float.floatToIntBits(key[p]) )) p = (p + 1) & mask;
	    return p;
	   }
	...
	private final class MapEntrySet extends AbstractObjectSet<Float2ObjectMap.Entry <V> > implements FastEntrySet <V> {
	 ...
	 public boolean contains(final Object o) {
	  ...
	  if (( Float.floatToIntBits(k) == 0 )) return Float2ObjectOpenHashMap.this.containsNullKey && java.util.Objects.equals(value[n], v);
	  float curr;
	  final float[] key = Float2ObjectOpenHashMap.this.key;
	  int pos;
	  // The starting point.
	  if (( Float.floatToIntBits(curr = key[pos = it.unimi.dsi.fastutil.HashCommon.mix( it.unimi.dsi.fastutil.HashCommon.float2int(k) ) & mask]) == 0 )) return false;
	  if (( Float.floatToIntBits(k) == Float.floatToIntBits(curr) )) return java.util.Objects.equals(value[pos], v);
	  // There's always an unused entry.
	  while(true) {
	   if (( Float.floatToIntBits(curr = key[pos = (pos + 1) & mask]) == 0 )) return false;
	   if (( Float.floatToIntBits(k) == Float.floatToIntBits(curr) )) return java.util.Objects.equals(value[pos], v);
	...
	private final class KeySet extends AbstractFloatSet {
	 ...
	 @Override
	 public void clear() { Float2ObjectOpenHashMap.this.clear();}
	}
	@Override
	...
	protected void rehash(final int newN) {
	 ...
	 for(int j = realSize(); j-- != 0;) {
	  ...
	  if (! ( Float.floatToIntBits(newKey[pos = it.unimi.dsi.fastutil.HashCommon.mix( it.unimi.dsi.fastutil.HashCommon.float2int(key[i]) ) & mask]) == 0 ))
