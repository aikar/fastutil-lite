...
public final class FloatArrays {
	...
	public static float[] trim(final float[] array, final int length) {
	 ...
	 final float t[] =
	  length == 0 ? EMPTY_ARRAY : new float[length];
	...
	private static void selectionSort(final float[] a, final float[] b, final int from, final int to) {
	 for(int i = from; i < to - 1; i++) {
	  ...
	  for(int j = i + 1; j < to; j++)
	   if ((u = ( Float.compare((a[j]),(a[m])) )) < 0 || u == 0 && ( Float.compare((b[j]),(b[m])) < 0 )) m = j;
	  if (m != i) {
	   float t = a[i];
	   a[i] = a[m];
	   a[m] = t;
	 ...
	 * of {@code a} in the specified range.
	 */
	public static void mergeSort(final float a[], final int from, final int to, FloatComparator comp, final float supp[]) {
	 int len = to - from;
	 // Insertion sort on smallest arrays
	 if (len < MERGESORT_NO_REC) {
	...
	private static final class ArrayHashStrategy implements Hash.Strategy<float[]>, java.io.Serializable {
	 ...
	 @Override
	 public int hashCode(final float[] o) { return java.util.Arrays.hashCode(o); }
	 @Override
	 public boolean equals(final float[] a, final float[] b) { return java.util.Arrays.equals(a, b); }
	}
	/** A type-specific content-based hash strategy for arrays.
	 *
