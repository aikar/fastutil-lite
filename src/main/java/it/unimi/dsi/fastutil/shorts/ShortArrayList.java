...
public class ShortArrayList extends AbstractShortList implements RandomAccess, Cloneable, java.io.Serializable {
	 ...
	 */
	protected ShortArrayList(final short a[], @SuppressWarnings("unused") boolean dummy) {
	 this.a = a;
	}
	...
	public void ensureCapacity(final int capacity) {
	 if (capacity <= a.length || a == ShortArrays.DEFAULT_EMPTY_ARRAY) return;
	...
	public boolean removeAll(final Collection<?> c) {
	 ...
	 int j = 0;
	 for(int i = 0; i < size; i++)
	  if (! c.contains(Short.valueOf(a[i]))) a[j++] = a[i];
	 final boolean modified = size != j;
	 size = j;
	 return modified;
	}
