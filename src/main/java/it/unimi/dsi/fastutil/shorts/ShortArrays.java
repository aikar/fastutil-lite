...
public final class ShortArrays {
	...
	public static short[] trim(final short[] array, final int length) {
	 ...
	 final short t[] =
	  length == 0 ? EMPTY_ARRAY : new short[length];
	...
	private static void selectionSort(final short[] a, final short[] b, final int from, final int to) {
	 for(int i = from; i < to - 1; i++) {
	  ...
	  for(int j = i + 1; j < to; j++)
	   if ((u = ( Short.compare((a[j]),(a[m])) )) < 0 || u == 0 && ( (b[j]) < (b[m]) )) m = j;
	  if (m != i) {
	   short t = a[i];
	   a[i] = a[m];
	   a[m] = t;
	 ...
	 * of {@code a} in the specified range.
	 */
	public static void mergeSort(final short a[], final int from, final int to, ShortComparator comp, final short supp[]) {
	 int len = to - from;
	 // Insertion sort on smallest arrays
	 if (len < MERGESORT_NO_REC) {
	...
	private static final class ArrayHashStrategy implements Hash.Strategy<short[]>, java.io.Serializable {
	 ...
	 @Override
	 public int hashCode(final short[] o) { return java.util.Arrays.hashCode(o); }
	 @Override
	 public boolean equals(final short[] a, final short[] b) { return java.util.Arrays.equals(a, b); }
	}
	/** A type-specific content-based hash strategy for arrays.
	 *
