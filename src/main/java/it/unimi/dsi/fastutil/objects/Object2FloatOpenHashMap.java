...
public class Object2FloatOpenHashMap <K> extends AbstractObject2FloatMap <K> implements java.io.Serializable, Cloneable, Hash {
	...
	private final class MapEntrySet extends AbstractObjectSet<Object2FloatMap.Entry <K> > implements FastEntrySet <K> {
	 ...
	 public boolean contains(final Object o) {
	  ...
	  if (( (k) == null )) return Object2FloatOpenHashMap.this.containsNullKey && ( Float.floatToIntBits(value[n]) == Float.floatToIntBits(v) );
	  K curr;
	  final K[] key = Object2FloatOpenHashMap.this.key;
	  int pos;
	  // The starting point.
	  if (( (curr = key[pos = ( it.unimi.dsi.fastutil.HashCommon.mix( (k).hashCode() ) ) & mask]) == null )) return false;
	  if (( (k).equals(curr) )) return ( Float.floatToIntBits(value[pos]) == Float.floatToIntBits(v) );
	  // There's always an unused entry.
	  while(true) {
	   if (( (curr = key[pos = (pos + 1) & mask]) == null )) return false;
	   if (( (k).equals(curr) )) return ( Float.floatToIntBits(value[pos]) == Float.floatToIntBits(v) );
	...
	private final class KeySet extends AbstractObjectSet <K> {
	 ...
	 @Override
	 public void clear() { Object2FloatOpenHashMap.this.clear();}
	}
	@Override
